...
namespace AssetStudio
{
    public static class CABManager
    {
        ...
        public static void BuildMap(List<string> files, int version)
        {
            ...
            try
            {
                ...
                var outputFile = new FileInfo($"Maps/PGRMap_v{version}.bin");
        ...
        public static void LoadMap(int version)
        {
            Logger.Info(string.Format("Loading PGRMap"));
            try
            {
                CABMap.Clear();
                using (var binaryFile = File.OpenRead($"Maps/PGRMap_v{version}.bin"))
